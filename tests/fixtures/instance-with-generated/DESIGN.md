...
Appended during the course. When two replicas return different values for one
cell, the client picks the highest write timestamp, and breaks a tie on the
replica id. This decision was made with the learner in lesson 03 and is not in
the bundle's DESIGN.md.
